...
export default function AssetCard({ asset, isFavorite, onOpen, onFavorite }: Props) {
  return (
    ...
    >
      <div className={`absolute top-0 left-0 right-0 h-0.5 rounded-t-2xl opacity-0 group-hover:opacity-100 transition-opacity duration-150
        ${asset.type === 'kpi' ? 'bg-blue-500' : ''}
        ${asset.type === 'layout' ? 'bg-green-500' : ''}
        ${asset.type === 'dataviz' ? 'bg-amber-500' : ''}
        ${asset.type === 'storyboard' ? 'bg-fuchsia-500' : ''}
      `} />
